...
exports = module.exports = function convert(input_path, quality, callback) {
  ...
  fs.exists(input_path, function (exists) {
    ...
    // var cmd = 'convert -depth 8 -background white -flatten +matte -density '+pdf_convert_quality+' "'+ input_path +'"  "' + output_path+'"';  "-dNumRenderingThreads=8 -dBandHeight=100 -dBandBufferSpace=500000000 -sBandListStorage=memory -dBufferSpace=1000000000 " +
    var child = exec(cmd, function (err, stderr, stdout) {
      console.log('page:', Number(input_path.split('/page').pop().toLowerCase().split('.pdf').shift()));
      if (err) {
        return callback(err);
      }
