...
module.exports = function(input_path, options, callback) {
  ...
  fs.exists(input_path, function (exists) {
    ...
    const inputAsArray = input_path.split('/')
    inputAsArray.pop();
    // console.log(input_path)
    console.log(`${output_path.split('/').pop()}`)
    // console.log(`/private${inputAsArray.join('/')}`)
    var cmd = `docker run --rm -v ${inputAsArray.join('/')}/:/mnt tesseract:0.1 '/mnt/${input_path.split('/').pop()}' '/mnt/${output_path.split('/').pop()}' ${options.join(' ')}`;
